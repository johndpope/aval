...
export function validateLicensePolicy(input) {
  ...
  for (const key of ["allowed", "reviewRequired", "denied"]) if (!Array.isArray(input[key]) || new Set(input[key]).size !== input[key].length || input[key].some((value) => typeof value !== "string" || !/^[A-Za-z0-9.-]{1,64}$/u.test(value))) throw new Error(`license policy ${key} is invalid`);
...
function assertAllowed(records, policy) {
  ...
  for (const record of records) {
    ...
    if (record.license === null) failures.push(`${record.path}: unknown license`);
    else if (denied.has(record.license)) failures.push(`${record.path}: denied license ${record.license}`);
    else if (reviewed.has(record.license)) failures.push(`${record.path}: license ${record.license} requires an explicit policy record`);
    else if (!allowed.has(record.license)) failures.push(`${record.path}: unapproved license ${record.license}`);
  }
