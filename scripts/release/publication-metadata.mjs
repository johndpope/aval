...
export function validateApprovedPublicationMetadata(input) {
  ...
  if (authority.scope !== "@rendered-motion" || authority.registryUrl !== "https://registry.npmjs.org/" || typeof authority.owner !== "string" || !ACCOUNT.test(authority.owner) || typeof authority.evidenceId !== "string" || !REVIEW_ID.test(authority.evidenceId)) throw new Error("publication registry-scope authority is incomplete");
...
export function applyApprovedPublicationMetadata(source, input) {
  ...
  if (source === null || typeof source !== "object" || Array.isArray(source) || typeof source.name !== "string" || !source.name.startsWith("@rendered-motion/")) throw new Error("publication metadata package source is invalid");
  const directory = source.name.slice("@rendered-motion/".length);
  return {
    ...source,
