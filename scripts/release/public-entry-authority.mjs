...
export function reconcileProductionPublicEntryManifest(input, packageInspection) {
  ...
  if (packageInspection === null || typeof packageInspection !== "object" || !Array.isArray(packageInspection.packages) || packageInspection.packages.length !== 5) throw new Error("package inspection cannot authorize production public entries");
  const packages = new Map(packageInspection.packages.map((entry) => [entry?.name, entry]));
  if (packages.size !== 5) throw new Error("package inspection contains duplicate public package identities");
