...
const fixtureSource = resolve("fixtures/conformance/m8/user-states-all-routes-alpha.rma");
...
describe("candidate fixture authority stable reads", () => {
  it("rejects a symlink even when its target bytes match the manifest", async () => {
    ...
    try {
      ...
      await symlink(fixtureSource, join(root, "fixtures", "motion.rma"));
      await expect(load(root, artifact(bytes))).rejects.toThrow(/symbolic links/u);
    } finally { await rm(root, { recursive: true, force: true }); }
  });

  it("rejects format-oversize declarations before reading or allocating", async () => {
    const root = await temporaryRoot("oversize");
    try {
      const declared = { ...artifact(new Uint8Array(0)), byteLength: 32 * 1024 * 1024 + 1 };
...
async function temporaryRoot(label: string): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), `rma-candidate-fixture-${label}-`));
