...
const approved = {
  ...
  repositoryUrl: "https://github.com/acme/rendered-motion.git",
  homepageUrl: "https://github.com/acme/rendered-motion",
  bugsUrl: "https://github.com/acme/rendered-motion/issues",
...
describe("publication metadata authority", () => {
  it("injects one reviewed authority into every package-specific repository path", () => {
    ...
    const manifests = ["graph", "format", "player-web", "element", "compiler"].map((name) => applyApprovedPublicationMetadata({ name: `@rendered-motion/${name}` }, approved));
  ...
  it("fails closed while pending and rejects placeholder, credential, or scope substitutions", () => {
    ...
    expect(() => validateApprovedPublicationMetadata({ ...approved, bugsUrl: "https://user:secret@github.com/acme/rendered-motion/issues" })).toThrow(/canonical public HTTPS/u);
