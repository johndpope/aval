...
import type { BrowserAvcPlaybackSnapshot } from "./browser-avc-candidate-contracts.js";
import {
...
type BrowserPreparedTick = IntegratedPreparedContentTick & {
  ...
  readonly owner: BrowserAvcPlaybackSession;
...
export class BrowserAvcPlaybackSession
  ...
  public static async create(options: {
    ...
    readonly activation: Readonly<AvcCandidateActivationInput>;
    readonly hub: BrowserAvcCandidateHub;
  }): Promise<BrowserAvcPlaybackSession> {
    const session = new BrowserAvcPlaybackSession(options);
    try {
  ...
  public snapshot(): Readonly<BrowserAvcPlaybackSnapshot> {
...
function browserPlaybackAbortReason(signal: AbortSignal): unknown {
  return signal.reason ?? new DOMException(
    "browser AVC playback activation was aborted",
    "AbortError"
  );
}

/** @deprecated Use BrowserAvcPlaybackSession. */
export { BrowserAvcPlaybackSession as BrowserOpaquePlaybackSession };
