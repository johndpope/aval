...
/** @internal Testable owner of the browser decoder readiness sequence. */
  ...
  public async measure(
    ...
    while (requestedOffset < frames.length || expected.length > 0) {
      ...
      if (
        ...
        const requirement = this.#input.samples.nextGroupRequirement(
          request
        );
        assertReadinessGroupWithinLimits(requirement, this.#input.limits);
        const unitFrameCount = readinessUnitFrameCount(
          this.#input.context.catalog.manifest.units,
          request.unitId
        );
        const groupEnd = request.unitFrame + requirement.frameCount;
        if (groupEnd > unitFrameCount) {
        ...
        const outstandingFrames = checkedReadinessOutstandingFrames(
          metrics.submittedFrames,
          metrics.leasedFrames
        );
        if (hasReadinessSubmissionCredit(
          requirement,
          ...
          nextRequest = groupEnd < unitFrameCount
            ? Object.freeze({
                unitId: request.unitId,
                unitFrame: groupEnd
              })
            : frames[requestedOffset] ?? null;
          reorderLookahead = nextRequest !== null &&
            nextRequest.unitId === requirement.unitId &&
            groupEnd < unitFrameCount &&
      ...
      throw new RangeError("readiness probe cannot make progress within worker credit");
    }
    return recorder.report().frames;
  }
}

function planReadinessGroup(
  first: Readonly<WorkerSampleFrameRequest>,
  requirement: Readonly<WorkerSampleGroupRequirement>,
  requested: readonly Readonly<WorkerSampleFrameRequest>[],
  ...
  for (let index = 0; index < requirement.frameCount; index += 1) {
    const dependency = Object.freeze({
      unitId: first.unitId,
      unitFrame: first.unitFrame + index
    });
    const desired = requested[offset];
...
function assertReadinessGroupWithinLimits(
  requirement: Readonly<WorkerSampleGroupRequirement>,
  limits: Readonly<VideoCandidateReadinessSessionInput["limits"]>
): void {
  if (
    requirement.frameCount > limits.maxOutstandingFrames ||
    requirement.chunkCount > limits.maxPendingSamples
  ) {
    throw new RangeError("readiness codec group exceeds worker limits");
  }
}

function hasReadinessSubmissionCredit(
  requirement: Readonly<WorkerSampleGroupRequirement>,
  pendingSamples: number,
  outstandingFrames: number,
  limits: Readonly<VideoCandidateReadinessSessionInput["limits"]>
): boolean {
  return pendingSamples <= limits.maxPendingSamples - requirement.chunkCount &&
    outstandingFrames <=
      limits.maxOutstandingFrames - requirement.frameCount;
}

function checkedReadinessOutstandingFrames(
...
function remainingMs(input: Readonly<VideoCandidateReadinessSessionInput>): number {
