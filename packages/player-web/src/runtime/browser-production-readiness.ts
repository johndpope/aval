...
import {
  ...
  assertReadinessActive,
  type BrowserReadinessCleanup,
 ...
 * throughput probe, while this readiness is the semantic authority for every
 * graph route and resident/streaming ownership boundary.
 */
export class BrowserProductionReadiness {
  readonly #input: Readonly<VideoCandidateReadinessSessionInput>;
  readonly #hub: BrowserVideoCandidateHub;
  readonly #ringCapacity: number;
  readonly #cleanups: Readonly<BrowserReadinessCleanup>[] = [];
  ...
  public async run(): Promise<Readonly<BrowserProductionReadinessReport>> {
    assertReadinessActive(this.#input);
    const motionPolicy = assessProductionMotionPolicy();
  ...
  async #measureInverse(
    ...
    return this.#withDriver(`inverse:${unit}`, async (driver) => {
      ...
      let before: BrowserReadinessTick | null = null;
      ...
      let endpointFrame: BrowserReadinessTick | null = null;
      for (let count = 0; count < driver.maxTicks; count += 1) {
        const tick = await driver.tick();
        if (
          tick.media.graphKind === "reversible" &&
  ...
  async #withDriver<T>(
    label: string,
    operation: (driver: BrowserReadinessDriver) => Promise<T>
  ): Promise<T> {
    const startedMs = this.#input.clock.now();
    assertReadinessActive(this.#input);
    const driver = await BrowserReadinessDriver.create({
      candidate: this.#input,
      hub: this.#hub,
      ringCapacity: this.#ringCapacity
    });
