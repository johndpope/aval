...
describe("publication ledger", () => {
  it("is read-before-write and accepts only an exact immutable existing version", () => {
    const operation = planExactPublication({
      ...
      desiredTag: "next",
      registry: { name: "@rendered-motion/graph", version: "1.0.0", integrity, tags: { next: "1.0.0" } },
      sequence: 1,
    ...
    expect(() => planExactPublication({
      ...
      registryIntegrity: integrity,
      desiredTag: "next",
      registry: { name: "@rendered-motion/graph", version: "1.0.0", integrity: `sha512-${Buffer.alloc(64, 2).toString("base64")}`, tags: {} },
      sequence: 1,
      timestamp: "2026-07-12T13:00:00.000Z",
      approvalId: "approval-1"
    ...
    const tagPlan = planExactPublication({
      ...
      registryIntegrity: integrity,
      desiredTag: "next",
      registry: { name: "@rendered-motion/graph", version: "1.0.0", integrity, tags: {} },
      sequence: 1,
      timestamp: "2026-07-12T13:00:00.000Z",
      approvalId: "approval-1"
    });
    ...
    expect(tagPlan.beforeStateDigest).toBe(registryStateDigest({ name: "@rendered-motion/graph", version: "1.0.0", integrity, tags: {} }));
  });
  ...
  it("blocks latest promotion after a partial next publication and rolls back in reverse dependency order", () => {
    const planned = planExactPublication({
      packageName: "@rendered-motion/graph", version: "1.0.0", tarballSha256: "a".repeat(64), registryIntegrity: integrity,
      desiredTag: "next", registry: { name: "@rendered-motion/graph", version: "1.0.0", integrity: null, tags: {} },
      sequence: 1, timestamp: "2026-07-12T13:00:00.000Z", approvalId: "approval"
    });
    expect(() => assertPromotionAllowed(ledger([failPublicationOperation(planned)], { status: "failed" }), ["@rendered-motion/graph"])).toThrow(/passed executed/u);
  ...
  it("closes integrity, approval, registry-tag, and phase semantics", () => {
    ...
    expect(() => planExactPublication({
      packageName: "@rendered-motion/graph", version: "1.0.0", tarballSha256: "a".repeat(64), registryIntegrity: "sha512-ZA==",
      desiredTag: "next", registry: { name: "@rendered-motion/graph", version: "1.0.0", integrity: null, tags: {} },
  ...
  it("records an exact initial-release rollback as latest/next removal plus deprecation", () => {
    ...
    expect(operations.map(({ packageName, action, tag, after }) => ({ packageName, action, tag, after })).slice(0, 3)).toEqual([
      { packageName: "@rendered-motion/compiler", action: "rollback-tag", tag: "latest", after: null },
      { packageName: "@rendered-motion/compiler", action: "rollback-tag", tag: "next", after: null },
      { packageName: "@rendered-motion/compiler", action: "deprecate", tag: "deprecated", after: "Withdrawn" }
    ]);
...
function plannedReleaseSet() {
  return ["graph", "format", "player-web", "element", "compiler"].map((short, index) => planExactPublication({
    packageName: `@rendered-motion/${short}`,
    version: "1.0.0",
    tarballSha256: String(index + 1).repeat(64),
    registryIntegrity: integrity,
    desiredTag: "next",
    registry: { name: `@rendered-motion/${short}`, version: "1.0.0", integrity: null, tags: {} },
    sequence: index + 1,
    timestamp: "2026-07-12T13:00:00.000Z",
...
function initialReleaseRollbackOperations() {
  ...
  for (const short of ["compiler", "element", "player-web", "format", "graph"]) {
    const packageName = `@rendered-motion/${short}`;
    const base = {
      packageName,
      version: "1.0.0" as const,
      tarballSha256: "a".repeat(64),
