import { h265Invalid } from "./failure.js";
...
export class H265RbspBitReader {
  ...
  public skipBits(width: number, label: string): void {
    if (!Number.isSafeInteger(width) || width < 0 || this.bitsRemaining < width) {
      this.#fail(`truncated ${label}`);
    }
  ...
  public moreRbspData(): boolean {
    ...
    if (!this.#peekBit(this.#bitOffset)) return true;
    for (
      let bit = this.#bitOffset + 1;
      bit < this.#bytes.length * 8;
      bit += 1
    ) {
      if (this.#peekBit(bit)) return true;
    }
    return false;
  }
